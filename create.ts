...
export async function create(rootPath: string, configContent: Config | null) {
  ...
  const buildConfig = await detectBuildConfig(new FrameworkFileSystemReader(rootPath));
  ...
  const appCreationPromise = new Promise<{ org: string; app: string; }>((resolve, reject) => {
    const interval = setInterval(async () => {
      const res = await fetch(`${deployUrl!}/api/device_create/${deviceCreateId}`, {
        method: "GET",
      });
  ...
  const [{ org, app }, newToken] = await Promise.all([appCreationPromise, deployToken ? undefined : tokenExchange(exchangeToken!, verifier!, spinner)]);
