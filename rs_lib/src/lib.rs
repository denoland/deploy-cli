...
#[wasm_bindgen]
pub fn resolve_config_with_deploy_config(
  root_path: String,
  ignore_paths: Vec<String>,
) -> Result<JsValue, JsValue> {
  let result = inner_resolve_config_with_deploy_config(root_path, ignore_paths);
  result
    .map_err(|err| create_js_error(&err))
    .map(|val| serde_wasm_bindgen::to_value(&val).unwrap())
}

fn inner_resolve_config_with_deploy_config(
  root_path: String,
  ignore_paths: Vec<String>,
) -> Result<Option<ConfigLookup>, anyhow::Error> {
  ...
  if let Some(deno_json) = workspace_dir.member_or_root_deno_json() {
    if let Some(mut config) = deno_json.to_deploy_config()? {
      ...
      let files = collect_files(&real_sys, config.files);

      return Ok(Some(ConfigLookup {
        config: deno_json.specifier.to_string(),
        files,
      }));
    }
  }
...
pub fn resolve_config(
  ...
  let result = inner_resolve_config(root_path, ignore_paths);
  result
    .map_err(|err| create_js_error(&err))
    .map(|val| serde_wasm_bindgen::to_value(&val).unwrap())
}
...
fn inner_resolve_config(
  ...
  if let Some(deno_json) = workspace_dir.member_or_root_deno_json() {
    ...
    let files = collect_files(&real_sys, files_config);
    return Ok(Some(ConfigLookup {
      config: deno_json.specifier.to_string(),
      files,
    }));
