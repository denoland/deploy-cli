...
pub fn resolve_config_with_deploy_config(root_path: String) -> Result<Option<String>, JsValue> {
  let result = inner_resolve_config_with_deploy_config(root_path);
  result.map_err(|err| create_js_error(&err))
}


fn inner_resolve_config_with_deploy_config(root_path: String) -> Result<Option<String>, anyhow::Error> {
...
#[wasm_bindgen]
pub fn resolve_config(root_path: String) -> Result<Option<String>, JsValue> {
  let result = inner_resolve_config(root_path);
  result.map_err(|err| create_js_error(&err))
}

fn inner_resolve_config(root_path: String) -> Result<Option<String>, anyhow::Error> {
  let real_sys = sys_traits::impls::RealSys;
  let root_path = resolve_absolute_path(root_path)?;
  let root_paths = [root_path];
  let workspace_dir = WorkspaceDirectory::discover(
